...
let handler = async (m, { conn, text, usedPrefix, command }) => {
    ...
    try {
        await m.reply(`Analizando con *Deepseek*... 🧠✨`);

        const { consolidatedMessage, imageBuffer } = await solveFinancialProblem('deepseek', text);
        ...
        console.error('Error en el comando resolver5:', error);
        await conn.reply(m.chat, `Lo siento, ocurrió un error al procesar tu solicitud con Deepseek.\n\n*Detalles:*\n\`\`\`${error.message}\`\`\``, m);
    }
};

handler.help = ['resolver5 <problema>'];
handler.tags = ['tools'];
handler.command = /^(resolver5)$/i;
