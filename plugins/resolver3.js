...
let handler = async (m, { conn, text, usedPrefix, command }) => {
    ...
    try {
        await m.reply(`Analizando con *Mistral*... 🧠✨`);

        const { consolidatedMessage, imageBuffer } = await solveFinancialProblem('mistral', text);
        ...
        console.error('Error en el comando resolver3:', error);
        await conn.reply(m.chat, `Lo siento, ocurrió un error al procesar tu solicitud con Mistral.\n\n*Detalles:*\n\`\`\`${error.message}\`\`\``, m);
    }
};

handler.help = ['resolver3 <problema>'];
handler.tags = ['tools'];
handler.command = /^(resolver3)$/i;
