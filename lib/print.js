...
export default async function (m, conn = { user: {} }) {
    ...
    let oraPeru = oraAttuale.toLocaleString('es-PE', { hour: '2-digit', minute: '2-digit', second: '2-digit' });

    let chatName = chat ? (m.isGroup ? 'Grupo: ' + chat : 'Chat privado: ' + chat) : '';

    /* # Ignorar mensajes del bot */
    if (m.sender === conn.user?.jid) return;
    console.log(`${chalk.black(chalk.bgCyanBright('%s'))} ${chalk.whiteBright(oraPeru)}
